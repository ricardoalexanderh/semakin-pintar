// ===== Pathfinder Duel — WebRTC P2P Room System =====
// Own copy of the P2P system with a unique peer prefix to avoid collisions.

import Peer, { DataConnection } from 'peerjs';
import QRCode from 'qrcode';
import type { PDPeerMessage } from './types';

const PEER_PREFIX = 'pathfinder-';
...
type MessageHandler = (msg: PDPeerMessage) => void;
...
export class GameRoom {
  ...
  private init() {
    ...
    this.peer.on('error', (err) => {
      if (this.destroyed) return;
      console.warn('[Pathfinder Duel WebRTC] Peer error:', err.type, err.message);
  ...
  private setupConnection(conn: DataConnection) {
    ...
    conn.on('data', (data) => {
      ...
      const msg = data as PDPeerMessage;
    ...
    conn.on('error', (err) => {
      console.warn('[Pathfinder Duel WebRTC] Connection error:', err);
      const remotePeerId = conn.peer.replace(PEER_PREFIX, '');
      this.connections.delete(remotePeerId);
      if (!this.destroyed) this.onPeerDisconnected(remotePeerId);
    });
  ...
  broadcast(type: PDPeerMessage['type'], payload: unknown) {
    this.send({
      type,
      payload,
...
export async function generateQRCodeDataURL(roomCode: string): Promise<string> {
  ...
  return QRCode.toDataURL(joinUrl, {
    width: 360,
    margin: 2,
    color: { dark: '#22c55eff', light: '#0a1628ff' },
