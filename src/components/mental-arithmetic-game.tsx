...
const MentalArithmeticGame: React.FC = () => {
  ...
  const speak = (text: string): Promise<void> => {
    return new Promise((resolve) => {
      ...
      try {
        ...
        const estimatedDuration = text.length * 100 + 1000;
        speechTimeoutRef.current = setTimeout(resolveOnce, estimatedDuration);
  ...
  // Generate number and question functions
  ...
  const pauseGame = (): void => {
    ...
    setIsPaused(true);
    setGameState('paused');
    setTimeout(() => playSound('pause'), 100);
  };
  ...
  useEffect(() => {
    ...
    if (showingAnswer) {
      ...
      if (currentNumberIndex < currentNumbers.length) {
        const delay = settings.speechEnabled ? 3000 : 1500;
        const timer = setTimeout(() => {
          if (currentNumberIndex < currentNumbers.length - 1) {
  ...
  if (gameState === 'setup') {
    return (
      <div className={`min-h-screen bg-gradient-to-br ${currentTheme.setupBg} pt-4`}>
        <div className="p-4">
                  ...
                  {settings.speechEnabled && (
                    <div className="mt-3 p-3 bg-blue-100 rounded-lg">
                      <p className="text-sm text-blue-700">
                        ℹ️ Voice mode uses intelligent timing and offers 3 difficulty levels for optimal experience
                      </p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-indigo-50 rounded-2xl p-6">
                  ...
                  <div className={`grid gap-3 ${settings.speechEnabled ? 'grid-cols-1 sm:grid-cols-3' : 'grid-cols-2 sm:grid-cols-3 md:grid-cols-5'}`}>
                    {settings.speechEnabled ? (
                      ...
                      [1, 2, 3].map(level => {
                        const voiceLevelNames = ['Easy', 'Hard', 'Expert'];
                        const levelColors = [
  ...
  if (gameState === 'playing') {
    return (
      <div className={`min-h-screen bg-gradient-to-br ${currentTheme.playingBg} pt-4`}>
  ...
  if (gameState === 'paused') {
    return (
      <div className={`min-h-screen bg-gradient-to-br ${currentTheme.pausedBg} pt-4`}>
        <div className="flex items-center justify-center min-h-screen p-4">
          <div className={`${currentTheme.cardBg} rounded-3xl shadow-2xl p-12 text-center max-w-lg w-full`}>
            <div className="mb-8">
  ...
  if (gameState === 'results') {
    return (
      <div className={`min-h-screen bg-gradient-to-br ${currentTheme.resultsBg} pt-4`}>
        <div className="p-4">
