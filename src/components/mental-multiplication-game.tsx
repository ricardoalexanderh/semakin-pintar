...
import { Play, RefreshCw, ArrowLeft, X } from 'lucide-react';
...
interface GameSettings {
    multiplierDigits: 1 | 2 | 3 | 4;
    multiplicandDigits: 1 | 2 | 3 | 4;
    numQuestions: number;
    level: 1 | 2 | 3 | 4 | 5;
...
interface MentalMultiplicationGameProps {
    onBackToHome?: () => void;
}

const MentalMultiplicationGame: React.FC<MentalMultiplicationGameProps> = ({ onBackToHome }) => {
    ...
    const [settings, setSettingsState] = useState<GameSettings>(() => {
        try {
            const stored = localStorage.getItem('mental-multiplication-settings');
            if (stored) {
    ...
    const updateSettings = (newSettings: GameSettings | ((prev: GameSettings) => GameSettings)): void => {
        ...
        try {
            localStorage.setItem('mental-multiplication-settings', JSON.stringify(updatedSettings));
        } catch (error) {
    ...
    const generateQuestion = (): Question => {
        ...
        do {
            ...
            if (attempts > maxAttempts) {
                ...
                multiplier = generateNumber(settings.multiplierDigits);
                multiplicand = generateNumber(settings.multiplicandDigits);
                answer = multiplier * multiplicand;
                break;
            }

            // Generate multiplier with bias against 1 (make multiplier = 1 less frequent)
            if (settings.multiplierDigits === 1 && Math.random() < 0.1) {
                // Only 10% chance of getting multiplier = 1 for single digit
                multiplier = 1;
            } else {
                multiplier = generateNumber(settings.multiplierDigits);
                // Ensure we don't get multiplier = 1 when we don't want it
                while (multiplier === 1 && Math.random() < 0.9) {
                    multiplier = generateNumber(settings.multiplierDigits);
                }
            }
            ...
            // Avoid multiplication by 1 to make problems more interesting
            while (multiplicand === 1 && Math.random() < 0.9) {
                multiplicand = generateNumber(settings.multiplicandDigits);
            }
            ...
            // Optional: Add constraints on answer size if needed
            // For example, if you want to limit the maximum answer:
            /*const maxAnswer = 100000; // Adjust as needed
            if (answer > maxAnswer) {
                continue; // Try again with smaller numbers
            }*/

            // Break the loop since we have a valid multiplication problem
            break;
    ...
    useEffect(() => {
        ...
        updateGameState('mental-multiplication', isPlaying);
    ...
    const estimateSpeechDuration = (text: string): number => {
        ...
        const words = text.replace(' times ', '').replace(/\d+/g, '').split(/\s+/).filter(word => word.length > 0);
    ...
    const getDelays = (level: number) => {
        ...
        return {
            numberDelay: baseNumberDelay * 2, //Times 2, Because multiplication is only per question, not per number, so it needs more delay time
    ...
    const startGame = async (): Promise<void> => {
        ...
        // Generate questions for multiplication game        
        ...
        setTimeout(() => {
            trackGameEvent('mental-multiplication', 'start', {
                multiplierDigits: settings.multiplierDigits,
                multiplicandDigits: settings.multiplicandDigits,
        ...
        setTimeout(() => {
            setTimeout(() => {
                ...
                if (questions.length > 0) {
                    ...
                    setDisplayText(`${firstQuestion.multiplicand} x ${firstQuestion.multiplier}`);
                    ...
                    if (settings.speechEnabled) {
                        ...
                        setTimeout(() => {
                            setDisplayText(`${firstQuestion.multiplicand} x ${firstQuestion.multiplier}`);
                            speak(`${firstQuestion.multiplicand} times ${firstQuestion.multiplier}`);
                        }, firstNumberDelay);
                    } else {
                        setDisplayText(`${firstQuestion.multiplicand} x ${firstQuestion.multiplier}`);
                    }
    ...
    const pauseGame = (): void => {
        setTimeout(() => {
            trackGameEvent('mental-multiplication', 'pause', {
    ...
    const resumeGame = (): void => {
        setTimeout(() => {
            trackGameEvent('mental-multiplication', 'resume', {
                currentQuestion: currentQuestion + 1,
                totalQuestions: settings.numQuestions
            });
        }, 0);
    ...
    const completeGame = (): void => {
        ...
        setTimeout(() => {
            trackGameEvent('mental-multiplication', 'complete', {
                numQuestions: settings.numQuestions,
    ...
    // Enhanced game logic effect for multiplication
    useEffect(() => {
        ...
        if (showingAnswer) {
                        ...
                        if (currentQuestion < allQuestions.length - 1) {
                            ...
                            setTimeout(() => {
                                ...
                                setTimeout(() => {
                                    ...
                                    setDisplayText(`${nextQ.multiplicand} x ${nextQ.multiplier}`);

                                    if (settings.speechEnabled) {
                                        speak(`${nextQ.multiplicand} times ${nextQ.multiplier}`);
                                    }
                                }, 800);
                            }, 1000);
                        } else {
            ...
            if (settings.speechEnabled) {
                const speechText = `${currentQ.multiplicand} times ${currentQ.multiplier}`;
                ...
                <div className="max-w-2xl mx-auto">
                    <div className={`${currentTheme.cardBg} rounded-3xl shadow-2xl p-8`}>
                        ...
                        {onBackToHome && (
                            <button
                                onClick={() => {
                                    trackButtonClick('back-to-home', 'mental-multiplication-setup');
                        ...
                        <div className="text-center mb-8">
                            <X className={`w-16 h-16 mx-auto ${currentTheme.secondary} mb-4`} />
                            <h1 className={`text-4xl font-bold ${currentTheme.primary} mb-2`}>Mental Multiplication Game</h1>
                            <p className={`text-xl ${currentTheme.secondary}`}>Master multiplication with mental math!</p>
                        </div>
                        ...
                        <div className="space-y-6">
                                ...
                                <div className="flex items-center justify-center gap-2 sm:gap-4">
                                    ...
                                    <div className="flex flex-col items-center gap-3 flex-1">
                                            ...
                                            {[1, 2, 3, 4].map(digits => (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        updateSettings(prev => ({
                                                            ...prev,
                                                            multiplicandDigits: digits as GameSettings['multiplicandDigits'],
                                                            multiplierDigits: digits < prev.multiplierDigits ? digits as GameSettings['multiplierDigits'] : prev.multiplierDigits
                                                        }));
                                                    }}
                                                    className={`h-[35px] sm:h-[40px] rounded-xl text-lg sm:text-xl font-bold transition-all transform hover:scale-105 ${settings.multiplicandDigits === digits
                                        ...
                                        <div className="bg-white px-3 py-1 rounded-full border-2 border-green-200 shadow-sm h-[28px] flex items-center justify-center w-[60px] mt-2">
                                            <span className="text-xs sm:text-sm text-green-700 font-bold">
                                                {settings.multiplicandDigits === 1 ? 'digit' : 'digits'}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center justify-center px-2 sm:px-4">
                                        <div className="bg-white p-2 sm:p-3 rounded-full shadow-lg border-2 border-green-300">
                                            <X className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-green-600" />
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex flex-col items-center gap-3 flex-1">
                                            ...
                                            {[1, 2, 3, 4].map(digits => (
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        updateSettings(prev => ({
                                                            ...prev,
                                                            multiplierDigits: digits as GameSettings['multiplierDigits'],
                                                            multiplicandDigits: digits > prev.multiplicandDigits ? digits as GameSettings['multiplicandDigits'] : prev.multiplicandDigits
                                                        }));
                                                    }}
                                                    className={`h-[35px] sm:h-[40px] rounded-xl text-lg sm:text-xl font-bold transition-all transform hover:scale-105 ${settings.multiplierDigits === digits
                                        ...
                                        <div className="bg-white px-3 py-1 rounded-full border-2 border-blue-200 shadow-sm h-[28px] flex items-center justify-center w-[60px] mt-2">
                                            <span className="text-xs sm:text-sm text-blue-700 font-bold">
                                                {settings.multiplierDigits === 1 ? 'digit' : 'digits'}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <button
                                onClick={() => {
                                    trackButtonClick('start-game', 'mental-multiplication-setup');
                                    playSound('buttonClick');
                                    startGame();
                                }}
                    ...
                    {showingGetReady ? (
                        ...
                        <>
                            ...
                            <div className="flex gap-4 justify-center">
                                <button
                                    onClick={() => {
                                        trackButtonClick('pause', 'mental-multiplication-playing');
                                        playSound('buttonClick');
                                        pauseGame();
                                    }}
            ...
            <div className={`min-h-screen bg-gradient-to-br ${currentTheme.pausedBg} flex items-center justify-center p-4`}>
                <div className={`${currentTheme.cardBg} rounded-3xl shadow-2xl p-12 text-center max-w-lg w-full`}>
                    ...
                    <div className="flex gap-4 justify-center">
                        <button
                            onClick={() => {
                                trackButtonClick('resume', 'mental-multiplication-paused');
                                playSound('buttonClick');
                                resumeGame();
                            }}
                ...
                <div className="max-w-4xl mx-auto">
                    <div className={`${currentTheme.cardBg} rounded-3xl shadow-2xl p-8`}>
                        <div className="text-center mb-8">
                            ...
                            <p className="text-lg sm:text-xl md:text-2xl text-green-600 px-2">Here are all your multiplication problems and answers:</p>
                        </div>
                            ...
                            {allQuestions.map((question, index) => (
                                <div key={index} className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl p-4 sm:p-6">
                                    ...
                                    <div className="flex items-center justify-center gap-3 sm:gap-4 mb-2">
                                        <span className="text-xl sm:text-2xl font-bold text-purple-700">{question.multiplicand}</span>
                                        <X className="w-5 h-5 sm:w-6 sm:h-6 text-blue-600" />
                                        <span className="text-xl sm:text-2xl font-bold text-purple-700">{question.multiplier}</span>
...
export default MentalMultiplicationGame;